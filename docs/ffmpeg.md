...
Day 17 没有新增媒体依赖：缩略图复用
`@ffmpeg-installer/win32-x64@4.1.0`（GPLv3），音频时长复用
`@ffprobe-installer/win32-x64@5.1.0`（GPL-3.0）。二者已被
`electron-builder.yml` 作为 `media/ffmpeg.exe` 和
`media/ffprobe.exe` sidecar 收入 Windows 包，并通过
`resolveMediaToolPaths()` 解析生产路径。该选择避免引入新的 Electron
native module 和 ABI/重建风险；许可证与再分发义务沿用现有 sidecar
台账，不因 Day 17 增加新的包。
