...
> **工单编号**：B-35/45<br>
> **角色**：Engineer<br>
> **来源**：`DAILY_PLAN.md` Day 35<br>
> **分支建议**：`chore/day-35-gate-c`<br>
> **任务类型**：核心技术验收 + 重复导出 + 一致性 Gate<br>
...
**整合验收 / Gate C。** 不新增能力；把 Day 31～34 的后台链路与唯一 ExportDialog 一起纳入生产证据。

### 工作区与状态验收补充

- 采集 ExportDialog 配置、校验、渲染、编码、取消/清理、失败与完成全状态截图，并证明同一时刻实例数 `<= 1`。
- 进度必须单调且阶段含义准确；关闭 Dialog 不取消任务，取消后 UI 可立即再次导出且无残留订阅/进程/temp。
- 导出期间主窗口保持响应；用户继续查看项目不会改变 ExportSnapshot，编辑只影响当前项目的正常 dirty，不影响运行中 job。
- 错误必须定位具体素材或阶段，不以原始 FFmpeg stderr 充当用户文案。

### 真实 Electron 验收补充

- 800×560 与 1366×768 下 Dialog 不裁切，键盘可完成配置、开始、取消确认、重试和关闭。
- 除原编码、关键帧、漂移、三次一致性、中文路径、内存与清理证据外，补充主窗口响应录屏和取消后再次导出录屏。
- Gate C FAIL 继续冻结 Day 36～45；不得用视觉完成度覆盖确定性、漂移、清理或恢复失败。

### Roadmap Evidence Index（A1～A22）

- **本日覆盖**：`A2 中文路径`、`A10 音频同步`、`A11 移动动画`、`A13 抖动`、`A17 视频导出`、`A18 预览/导出一致`、`A19 导出取消`。
- **反向证据**：Gate C 同一 Snapshot 的编码、关键帧差异、漂移、中文路径、三次一致性、取消/清理和 UI 状态证据。

### 追踪来源

- [原正式 Day 35 工单](../../../agent%20task/DAY-35-AGENT-TASK.md)
