...
> **工单编号**：B-29/45<br>
> **角色**：Engineer<br>
> **来源**：`DAILY_PLAN.md` Day 29<br>
> **分支建议**：`feat/day-29-project-preview`<br>
> **任务类型**：功能开发 + 多镜头调度 + 音频时钟<br>
...
**重点映射 / UI 状态机。** 连续预览的业务目标与 AudioContext 主时钟要求保持不变。

### 工作区、Owner 与状态边界

| 维度 | 补充要求 |
|---|---|
| 入口 | TopBar 提供“预览当前 / 预览全部”；TimelineToolbar 提供播放、暂停、停止与时间码。 |
| 画面 | `CanvasWorkspace` 使用同一 `CanvasStage` 切换预览态，暂时禁用编辑 handles；禁止生成第二棵全屏产品 DOM。 |
| 状态 | StatusBar 显示当前镜头、项目总时间、音频解锁/错误；PreviewStore 是唯一预览 UI 状态源。 |
| 持久化 | 播放、暂停、seek、当前总时间与预览模式不写项目、不进 History、不设置 dirty。 |
| 音频 | 只允许一个 AudioContext / 调度 Owner；seek、stop、切镜头必须立即停止旧节点。 |

默认状态机结束后回到 `editing@0` 并恢复编辑 handles；若主理人改为停在末尾，必须统一修改文档与测试，不能局部自创。

### 视觉与真实 Electron 验收补充

- 播放时持续可见当前镜头与总时间；状态不能仅靠按钮颜色，暂停/播放/错误均有文字或图标信号。
- 连续预览切镜头无上一镜头图层、字幕或音频残留；重播 5 次不叠音、不增长 AudioContext/节点。
- 800×560 与 1366×768 均能触达预览入口和控制；根页面不滚动，Timeline 可在自身 viewport 内滚动。
- 真实 Electron 走通预览当前→预览全部→暂停→seek→继续→停止→重播，并证明项目 snapshot、dirty 与 History 不变、焦点返回播放按钮。

### Roadmap Evidence Index（A1～A22）

- **本日覆盖**：`A10 音频同步`、`A16 完整预览`、`A18 预览/导出一致`。
- **反向证据**：五镜头连续播放、seek/stop、30 秒漂移、反复播放无叠音与共享 evaluator/renderer 证据。

### 追踪来源

- [原正式 Day 29 工单](../../../agent%20task/DAY-29-AGENT-TASK.md)
...
- **预计变更文件**：
  ...
  - `src/renderer/features/preview/ProjectPreviewController.ts`
  - `src/renderer/features/preview/AudioClock.ts`
  - `src/renderer/features/preview/AudioScheduler.ts`
  - `src/renderer/features/preview/PreviewControls.tsx`
  - `src/renderer/stores/previewStore.ts`
