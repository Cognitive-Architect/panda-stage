...
> **工单编号**：B-27/45<br>
> **角色**：Engineer<br>
> **来源**：`DAILY_PLAN.md` Day 27<br>
> **分支建议**：`feat/day-27-timeline-evaluator`<br>
> **任务类型**：领域逻辑 + 插值算法 + 冲突规则<br>
...
| 当前模型 | TimelineEvent union、Layer 初始状态、Shot duration 与旧 evaluator 可用 | `git grep -n "evaluateShotAtTime\|TimelineEvent\|move\|scale\|opacity\|shake\|expression\|visibility" -- src tests` | 必须 |
...
**轻度架构边界。** 本日仍以纯 domain evaluator 为主，不扩大为工作区重构。

### 工作区、Owner 与状态边界

| 维度 | 补充要求 |
|---|---|
| 正式消费位置 | 冲突结果只进入 Timeline clip 外观、Inspector 事件参数页 inline error 与 StatusBar 简短摘要。 |
| 唯一 Owner | domain evaluator 是时间求值真相；Preview 与 hidden export 必须复用同一 evaluator，不得各写一套。 |
| 临时状态 | 拖动候选位置属于 UI 临时状态；冲突时保留最后合法值，只有 pointer up 的合法结果才提交一次 History。 |
| 项目状态 | shake 只叠加基础位置；Inspector 不得把求值后的 shake 坐标写回项目 x/y。 |
| 禁止扩张 | 不增加 Evaluator 调试长页面，不建立第二套 Timeline/Inspector Owner。 |

### 验收补充

- evaluator 连续运行 100 次结果一致，稳定排序不依赖数组偶然顺序。
- 非法重叠在创建/拖动提交前阻止；错误 clip 可键盘聚焦并能定位到对应 Inspector。
- 冲突除颜色外还必须有图标或文本；沿用全局视觉 Token，不新增领域专属设计系统。
- 通过正式工作区的真实 Electron 探针证明结果被 Timeline / Inspector 消费；该探针只验证集成边界，不把 Day 27 偷换成 UI 重构。

### Roadmap Evidence Index（A1～A22）

- **本日覆盖**：`A11 移动动画`、`A12 表情切换`、`A13 抖动`、`A18 预览/导出一致`、`A21 自动化测试`。
- **反向证据**：evaluator/interpolator/conflict 单测、100 次确定性、共享 Preview/export evaluator 与正式 Timeline/Inspector 消费证据。

### 追踪来源

- [原正式 Day 27 工单](../../../agent%20task/DAY-27-AGENT-TASK.md)
...
- **预计变更文件**：
  ...
  - `src/domain/models/timeline-event.ts` 必要适配
