...
> **工单编号**：B-28/45<br>
> **角色**：Engineer<br>
> **来源**：`DAILY_PLAN.md` Day 28<br>
> **分支建议**：`feat/day-28-dialogue-subtitle-audio`<br>
> **任务类型**：功能开发 + 多媒体同步 + 降级容错<br>
...
| 当前模型 | Dialogue、AudioClip、SubtitleStyle、Character mouth asset、Asset duration 可用 | `git grep -n "Dialogue\|AudioClip\|SubtitleStyle\|mouthOpenAssetId\|durationMs" -- src tests` | 必须 |
...
**重点映射 / UI 功能。** 原对白、字幕、音频与嘴型目标不变；新增内容只明确正式工作区归属与交互合同。

### 工作区、Owner 与状态边界

| 能力 | 正式 Owner / 落位 | 状态边界 |
|---|---|---|
| Dialogue clip | `TimelineDock` 的 Dialogue Track | 选中/拖动候选为 UI 状态；pointer up 只提交一个 History。 |
| 新增对白 | `TimelineToolbar` 或轨道空状态 | 新增命令写项目并设置 dirty，不另建管理页。 |
| DialogueEditor | 右侧 `InspectorDock` | 页签切换不得丢未提交文本或串到其他项目；提交后才进入项目真相。 |
| 字幕 | 中央唯一 `CanvasStage` | 文本、时间与样式是项目状态；预览播放头不是。 |
| 音频 / 嘴型 | 对白 Inspector + clip 状态；domain evaluator / StageRenderer | 音频状态可见；嘴型不拥有独立页面或第二 Stage。 |

Inspector 信息结构沿用 PR #75：角色、文本、开始/时长、音频、字幕开关与样式，低频重叠规则放入“高级”并只读解释。

### 视觉与真实 Electron 验收补充

- 单一 Timeline、Inspector 和 Stage 完成“添加对白→编辑文本/时间→分配音频→预览字幕与嘴型”；不得滚动根页面寻找旧卡片。
- 1366×768 可完成完整流程；800×560 下 Inspector 与 Timeline 可内部滚动，关键按钮和安全区预览仍可达。
- 超长字幕实时显示为安全区内最多两行的可读预览；无音频、无嘴图、空文本、长文本均不白屏。
- 选中对白时 Stage 与音频预览定位到 `startMs`；保存重开后文本、时间、音频和样式一致。
- 状态、错误、选中与音频缺失不能只靠颜色；焦点、文字和图标遵循 `DESIGN.md`。

### Roadmap Evidence Index（A1～A22）

- **本日覆盖**：`A5 角色表情`、`A9 对白字幕`、`A12 表情切换`、`A14 嘴巴动画`。
- **反向证据**：对白/字幕/嘴型边界、表情稳定排序、保存重开和唯一 Stage 的真实 Electron 证据。

### 追踪来源

- [原正式 Day 28 工单](../../../agent%20task/DAY-28-AGENT-TASK.md)
