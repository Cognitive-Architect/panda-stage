...
> **工单编号**：B-33/45<br>
> **角色**：Engineer<br>
> **来源**：`DAILY_PLAN.md` Day 33<br>
> **分支建议**：`feat/day-33-audio-mix`<br>
> **任务类型**：功能开发 + FFmpeg 音频图 + 最终封装<br>
...
| 当前模型 | Dialogue、AudioClip、音量字段、Asset relativePath、project time mapping | `git grep -n "Dialogue\|AudioClip\|volume\|relativePath\|mapProjectTimeToShot\|FFmpegAdapter" -- src tests` | 必须 |
...
**轻度架构边界 / 后台编码。** 多音频混合与封装仍是 Main/FFmpeg 服务职责，不扩成混音台 UI。

### Owner 与状态边界

- Main export pipeline 拥有全局音频时间、filter graph、参数数组、stderr 与 jobId；Renderer 不复制混音逻辑。
- 唯一 ExportDialog 只把阶段从 rendering 切换为 encoding，并显示“正在混合音频 / 编码视频”和固定输出摘要 `1080p / 24 FPS / H.264 + AAC`。
- 缺失或损坏音频必须指名具体素材并给出下一步；用户界面不展示整条 FFmpeg 命令。
- 无音频项目的策略必须明确且可测试；技术日志按现有隐私规则脱敏。
- 编码进度、日志展开与输出路径属于 export job/UI 状态，不写项目、不进 History、不设置 dirty。

### 集成验收补充

- 不新增 Mixer 页面、后台服务面板或第二个 ExportDialog。
- 真实 Electron 验证 rendering→encoding→完成/失败的状态切换，主窗口保持响应，错误可恢复后重试。
- 窄窗口下阶段名、素材名与建议可换行但不撑破 Dialog；状态除颜色外有文字/图标。

### Roadmap Evidence Index（A1～A22）

- **本日覆盖**：`A17 视频导出`。
- **反向证据**：全局音频延迟/混合、H.264 + AAC 封装、无音频策略与媒体探针证据。

### 追踪来源

- [原正式 Day 33 工单](../../../agent%20task/DAY-33-AGENT-TASK.md)
...
- **预计变更文件**：
  ...
  - `src/main/services/AudioMixService.ts`
  - `src/main/services/FFmpegFilterGraphBuilder.ts`
  - `src/main/services/FFmpegAdapter.ts`
...
git rev-parse HEAD
git status --short
git grep -n "AudioMixService\|FFmpegFilterGraphBuilder\|adelay\|amix\|ffprobe" -- src tests docs
pnpm typecheck
pnpm lint
pnpm test:unit
