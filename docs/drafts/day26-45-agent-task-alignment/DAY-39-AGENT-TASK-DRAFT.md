...
> **工单编号**：B-39/45<br>
> **角色**：Engineer<br>
> **来源**：`DAILY_PLAN.md` Day 39<br>
> **分支建议**：`docs/day-39-release-docs`<br>
> **任务类型**：用户文档 + 开发文档 + 许可合规<br>
...
**整合验收 / 文档。** 本日只记录当时真实实现；PR #75 蓝图是输入，不得照抄为“已经完成”。

### 文档结构补充

- 用户指南按任务编排：建项目→准备素材/角色→组织镜头→舞台摆位→动作/对白→预览→保存/恢复/导出。
- 每步标注入口所在区域（TopBar、左 Dock、中央 Stage、右 Inspector、底部 Timeline、StatusBar），禁止继续写“向下滚找到某卡片”。
- 开发/架构文档记录每个能力的唯一 Owner、项目/UI/后台状态边界、dirty/History 语义、滚动 Owner 与单实例合同。
- 对仍未实现、Gate 未 PASS 或仅存在于 #75 的内容明确标注状态，不用未来截图或文案冒充当前产品。

### 验收补充

- 用真实 Electron 截图复核用户步骤；至少覆盖标准与窄窗口，敏感路径/个人信息先脱敏。
- 新用户能仅按文档打开演示并导出；新开发者能定位正式组件树而不会另建平行 UI 根。
- 本日文档变更不修改生产 UI；许可、已知问题与第三方清单继续按原工单验收。

### Roadmap Evidence Index（A1～A22）

- **本日覆盖**：无新增 A 项；本日是证据可发现性与接管文档 Owner。
- **反向证据**：用户/开发/架构文档必须逐项链接当时真实的 A1～A22 证据与状态，不得把蓝图当成完成证据。

### 追踪来源

- [原正式 Day 39 工单](../../../agent%20task/DAY-39-AGENT-TASK.md)
