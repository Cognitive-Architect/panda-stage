...
> **工单编号**：B-31/45<br>
> **角色**：Engineer<br>
> **来源**：`DAILY_PLAN.md` Day 31<br>
> **分支建议**：`feat/day-31-export-snapshot`<br>
> **任务类型**：功能开发 + 导出前校验 + 不可变快照<br>
...
| 当前能力 | ProjectSchema、ReferenceScanner、eventConflicts、项目总时长、24 FPS 常量、导出探针可用 | `git grep -n "ProjectSchema\|ReferenceScanner\|eventConflicts\|projectDuration\|FPS\|ExportService" -- src tests` | 必须 |
...
**轻度架构边界 / 导出入口。** 不可变 Snapshot 与校验仍由 Main/domain 拥有；前端只补正式入口和结构化结果消费。

### 工作区、Owner 与状态边界

| 维度 | 补充要求 |
|---|---|
| 入口 | TopBar 的导出动作打开唯一 `ExportDialog`；不得新建 Export 页面或挂进旧长页面。 |
| 校验 Owner | Main/domain 生成不可变 ExportSnapshot 和结构化问题；Renderer 不重算另一份真相。 |
| Dialog 状态 | 点击“开始导出”先进入 `validating`；本日不得伪造 rendering 百分比。 |
| 定位 | 问题按缺素材、引用错误、时间冲突、输出路径分组；“定位”选中对应镜头/图层/对白并打开正确 Inspector。 |
| 持久化 | 校验与定位属于任务/UI 状态，不修改项目、不进入 History、不设置 dirty；Snapshot 创建后不受后续编辑影响。 |

### 验收补充

- 每个问题显示对象名称、影响与可执行定位；错误不能只靠颜色。
- 校验失败不得创建 hidden window、temp 文件或 FFmpeg 进程；修复项目后可直接重新验证。
- 800×560 与 1366×768 下 Dialog 不裁切，键盘能完成配置、验证、定位与返回。
- 真实 Electron 证明“导出→校验失败→定位→修复→重新验证”，且只存在一个 ExportDialog、项目 dirty 仅由实际修复产生。

### Roadmap Evidence Index（A1～A22）

- **本日覆盖**：`A17 视频导出`。
- **反向证据**：不可变 Snapshot、结构化校验、总时长/帧数与校验失败零副作用证据。

### 追踪来源

- [原正式 Day 31 工单](../../../agent%20task/DAY-31-AGENT-TASK.md)
...
- **预计变更文件**：
  ...
  - `src/main/services/ExportValidator.ts`
  - `src/main/services/ExportSnapshotBuilder.ts`
  - `src/main/services/ExportJobFactory.ts`
...
git rev-parse HEAD
git status --short
git grep -n "ExportSnapshot\|ExportValidator\|totalFrames\|ExportJobFactory" -- src tests
pnpm typecheck
pnpm lint
pnpm test:unit
