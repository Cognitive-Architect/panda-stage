...
> **工单编号**：B-38/45<br>
> **角色**：Engineer<br>
> **来源**：`DAILY_PLAN.md` Day 38<br>
> **分支建议**：`build/day-38-windows-package`<br>
> **任务类型**：Windows 打包 + 干净环境验证 + 卸载保护<br>
...
| 当前构建 | electron-builder 配置、package scripts、FFmpegAdapter、资源路径、应用 metadata | `git grep -n "electron-builder\|extraResources\|FFmpegAdapter\|app.getPath\|productName" -- package.json electron-builder.* src tests` | 必须 |
...
**整合验收 / Windows 窗口。** 打包与干净机目标保持不变，补充窗口体验和正式工作区适配。

### Owner 与状态边界

- 窗口尺寸、位置、最大化状态属于 app settings，由 Electron window owner 管理；不得写入项目、History 或 dirty。
- 首次尺寸必须落在主显示器 work area 内；移除外接显示器后窗口回到可见区域。
- 安装目录只读时主题、图标和 Demo 资源仍能加载；系统字体缺失使用 `DESIGN.md` 规定的 fallback。
- 不在本日顺手加入自动更新、登录、遥测或第二套窗口框架。

### 真实打包应用验收补充

- 在干净 Windows 的真实安装包中验证 800×560 最小窗口、100% / 125% / 150% DPI、中文用户名、最大化恢复与多显示器撤除。
- TopBar、单一 Canvas、左右 Dock、Timeline、StatusBar 与 ExportDialog 在各档均可达；Dialog 不跑到屏幕外，根页面不出现业务滚动。
- 重新启动后仅恢复窗口偏好，不意外改变项目 dirty、选择、播放头或项目内容。

### Roadmap Evidence Index（A1～A22）

- **本日覆盖**：`A2 中文路径`、`A22 构建打包`。
- **反向证据**：干净 Windows 安装、中文用户名/路径、资源、DPI、卸载/重装与窗口恢复证据。

### 追踪来源

- [原正式 Day 38 工单](../../../agent%20task/DAY-38-AGENT-TASK.md)
...
- **预计变更文件**：
  ...
  - FFmpeg 打包资源配置
  - `src/main/services/ResourcePathService.ts`
  - `src/main/index.ts` 必要适配
...
git rev-parse HEAD
git status --short
git grep -n "electron-builder\|extraResources\|resourcesPath\|userData\|FFmpeg" -- package.json electron-builder.* src tests docs
pnpm typecheck
pnpm lint
pnpm test:unit
