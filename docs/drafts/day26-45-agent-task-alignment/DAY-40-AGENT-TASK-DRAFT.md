...
> **工单编号**：B-40/45<br>
> **角色**：Engineer<br>
> **来源**：`DAILY_PLAN.md` Day 40<br>
> **分支建议**：`release/day-40-rc1`<br>
> **任务类型**：RC 冒烟 + 功能冻结 + 发布闭环<br>
...
**整合验收 / RC1 冻结。** RC 日不得继续改布局或视觉；只有阻断主流程或数据安全的 P0 能进入。

### 冻结与验收补充

- 可接受的 UI P0 仅包括：焦点陷阱阻断流程、支持尺寸下关键操作不可见、保存/导出状态误导、误操作导致数据损失、键盘/屏幕阅读路径完全阻断。
- 间距、配色或一般 polish 进入 Backlog，不在 RC commit 顺手修。
- RC 证据必须对应同一 commit 和安装包，复核 `LegacyWorkspace = 0`、Canvas/History/Timeline 单实例、ExportDialog `<= 1`、根页面无滚动。
- 在 800×560、1366×768 与常见 DPI 完成演示项目、新建项目、保存重开、预览和导出；状态除颜色外有文字/图标。
- RC 冒烟不允许直接改 Store/JSON；窗口/Dock/播放头等 UI 状态不得污染项目 dirty。

### Roadmap Evidence Index（A1～A22）

- **本日覆盖**：`A1 项目生命周期`、`A4～A7`、`A9 对白字幕`、`A20 演示项目`、`A22 构建打包`。
- **反向证据**：同一 RC commit/安装包上的新建/保存/重开、编辑主链、演示、导出与干净安装冒烟。

### 追踪来源

- [原正式 Day 40 工单](../../../agent%20task/DAY-40-AGENT-TASK.md)
