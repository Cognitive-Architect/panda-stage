...
> **工单编号**：B-26/45<br>
> **角色**：Engineer<br>
> **来源**：`DAILY_PLAN.md` Day 26<br>
> **分支建议**：`feat/day-26-timeline-shell`<br>
> **任务类型**：功能开发 + 时间坐标 + 播放头交互<br>
...
| 当前能力 | Shot duration、PreviewStore、FPS 常量、当前镜头选择可用 | `git grep -n "durationMs\|PreviewStore\|FPS\|currentShot" -- src tests` | 必须 |
...
**重点映射 / UI 基础设施。** 原工单的时间轴业务目标保持不变；本补丁只把它安装到 PR #75 定义的正式工作区，并补齐迁移与验收边界。

### 工作区、Owner 与状态边界

| 维度 | 补充要求 |
|---|---|
| 正式落位 | `TimelineShell` 必须由底部 `TimelineDock` 承载；真实路径使用 `src/renderer/features/*` 与 `src/renderer/shell/*`，不得按示意路径新建第二套 UI 根。 |
| 唯一 Owner | `EditorShell` 拥有五区布局；`TimelineDock` 拥有时间轴；生产树中 `CanvasStage = 1`、`HistoryControls = 1`、`TimelineDock = 1`。 |
| UI 状态 | 播放头、缩放、横向滚动、Dock 高度/折叠与当前 Activity 属于 `WorkspaceUiStore` / Preview 状态，不进入 `project.json`、History，也不设置 dirty。 |
| 项目状态 | 事件开始/结束时间、删除与参数修改仍走正式 command/History；Day 26 只允许提交本日明确拥有的能力。 |
| 迁移边界 | `LegacyWorkspace = 0`；不得保留隐藏的第二套 Canvas/History 来兼容旧 Gate，不得把 Timeline 挂回旧纵向长页面。 |

### 开工预检补充

- 对事件 clip 可视化、选择、参数编辑、改时长/删除、拖动提交与 Dialogue/audio 编辑逐项记录 `implemented / missing / assigned / blocked`、Owner、测试和目标 Day。
- Day 30 Gate B 必需但尚未分配的能力必须建阻塞项；不得用手改 JSON 代替正式 UI。
- 现有动作预设迁入 Inspector，不重复实现 domain owner；拖动冲突提交由 Day 27 语义解锁。

### 视觉与真实 Electron 验收补充

- 1366×768 首屏同时可见 TopBar、中央舞台、左右 Dock、Timeline 与 StatusBar；根页面 `scrollTop = 0`，只允许各 Dock / TimelineViewport 内部滚动。
- 800×560 仍可选镜头、操作单一画布、打开 Inspector、控制折叠/展开时间轴；同时采集 1200×760、1920×1080，以及 Windows 100% / 150% DPI 证据。
- 时间轴刻度、播放头、选中与吸附不能只靠颜色；遵循 `DESIGN.md` 的时间数字、焦点环、控件密度和状态语义。
- 在真实 Electron 中完成“镜头→图层→动作→时间轴→点击/拖动播放头→缩放→切镜头”，并同时证明 root 无滚动、cardinality 正确、dirty/History 不变。

### Day 26 runtime component 测试基线

- Day 26 即引入 `jsdom`、`@testing-library/react`、`@testing-library/user-event` 与 `axe-core`（或等价适配），新增独立 `vitest.component.config.ts` 和 `test:component`；不得把这些写成当前已存在工具。
- 最小 suite 必须真实渲染 `WorkspaceLayout`，验证：shots/assets/characters 互斥；Activity 切换不卸载唯一 Canvas；Inspector 跟随选择；Timeline 折叠/调高/seek 不 dirty；800×560 的侧栏切换与焦点恢复；Canvas/History/Timeline cardinality。
- Pure unit、runtime component、integration 与真实 Electron smoke 各自保留边界；`@playwright/test` 在 Day 37 正式化，不能拿组件测试替代 IPC/窗口，也不能等到 Day 37 才首次验证 DOM。

### Roadmap Evidence Index（A1～A22）

- **本日覆盖**：`A7 画布摆放`、`A8 撤销重做`、`A15 动作预设`。
- **反向证据**：WorkspaceLayout runtime component 证据 + 单一 Canvas/History/Timeline cardinality + 动作→事件→History→保存重开 + 真实 Electron 流程。

### 追踪来源

- [原正式 Day 26 工单](../../../agent%20task/DAY-26-AGENT-TASK.md)
...
- **预计变更文件**：
  ...
  - `src/renderer/features/timeline/TimelineShell.tsx`
  - `src/renderer/features/timeline/TimelineRuler.tsx`
  - `src/renderer/features/timeline/TimelinePlayhead.tsx`
  - `src/renderer/features/timeline/TimelineToolbar.tsx`
  - `src/renderer/stores/timelineViewportStore.ts`
  - PreviewStore 必要适配文件
