...
> **工单编号**：B-32/45<br>
> **角色**：Engineer<br>
> **来源**：`DAILY_PLAN.md` Day 32<br>
> **分支建议**：`feat/day-32-export-scheduler`<br>
> **任务类型**：功能开发 + 多镜头帧调度 + 背压控制<br>
...
| 当前实现 | ExportService、隐藏窗口 IPC、StageRenderer、project time mapping、写帧队列 | `git grep -n "ExportService\|export-window\|StageRenderer\|mapProjectTimeToShot\|MAX_PENDING_FRAMES" -- src tests` | 必须 |
...
**轻度架构边界 / 后台调度。** 本日不增加工作区区域或用户调试页。

### Owner 与状态边界

- Main export scheduler / hidden renderer 是 job、背压与帧写入真相；诊断进入 job log。
- Renderer 的唯一 ExportDialog 只消费 `jobId`、`phase`、`completedFrames`、`totalFrames`、`fraction`；IPC 推送必须节流，不能每帧触发整棵工作区重渲染。
- 进度按已写入帧单调更新；不得用队列入列、估算或动画伪造完成量。
- ExportSnapshot 创建后不可变；用户查看或继续编辑项目不改变当前 job，也不设置额外 dirty。
- 关闭应用、切项目或再次导出与运行中 job 冲突时必须给出明确状态；不在本日复制第二个 Export Owner。

### 集成验收补充

- 真实 Electron 中导出期间主窗口保持可操作，Timeline/Dock 的查看不会影响快照。
- 800×560 与 1366×768 下进度和取消入口可达；根页面不因日志或进度增长而滚动。
- 证明 ExportDialog 单实例、进度消息订阅在任务结束后释放、项目/History 与当前 job 隔离。

### Roadmap Evidence Index（A1～A22）

- **本日覆盖**：`A17 视频导出`。
- **反向证据**：多镜头逐帧调度、已写帧进度、有界队列/背压与 Snapshot 隔离证据。

### 追踪来源

- [原正式 Day 32 工单](../../../agent%20task/DAY-32-AGENT-TASK.md)
...
- **预计变更文件**：
  ...
  - `src/main/services/ExportScheduler.ts`
  - `src/main/services/FrameWriteQueue.ts`
...
git rev-parse HEAD
git status --short
git grep -n "ExportScheduler\|FrameWriteQueue\|evaluateProjectAtTime\|MAX_PENDING_FRAMES" -- src tests
pnpm typecheck
pnpm lint
pnpm test:unit
