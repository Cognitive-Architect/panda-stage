...
> **工单编号**：B-36/45<br>
> **角色**：Engineer<br>
> **来源**：`DAILY_PLAN.md` Day 36<br>
> **分支建议**：`feat/day-36-demo-project`<br>
> **任务类型**：功能开发 + 首次使用引导 + 许可合规<br>
...
| 当前能力 | ProjectService、欢迎页、项目复制、素材/角色/镜头/动作、预览、导出可用 | `git grep -n "ProjectService\|Welcome\|demo\|StageRenderer\|ExportService" -- src tests` | 必须 |
...
**整合映射 / 首次使用。** 演示项目与首次引导目标保持不变；新增正式 StartScreen 归属和状态边界。

### 工作区、Owner 与状态边界

- `StartScreen` 只包含 PrimaryActions（新建/打开/打开演示）、RecentProjects 与 GettingStarted；项目打开后进入既有 EditorShell，不创建第二套编辑器。
- 四步 Checklist 从真实项目状态派生：导入素材、创建/选择角色、创建镜头并放到舞台、添加动作/对白并预览。
- Checklist 的跳过/再次显示属于 app settings，不写 ProjectSchema、不进入 History、不设置项目 dirty；用户可从帮助入口重新打开。
- 演示模板复制由 Main Service 唯一负责；取消目录选择不产生半成品，安装目录模板保持只读。

### 视觉与真实 Electron 验收补充

- 引导可跳过，不使用遮住全屏且无法退出的 tour；步骤完成、错误和跳过不只靠颜色。
- 800×560 与 1366×768 下主动作、最近项目和 Checklist 仍可达；StartScreen 自身内部处理溢出，不让根页面变成长业务页。
- 在打包后的真实 Electron 验证首次启动→打开演示→完成/跳过 Checklist→关闭重开；同时验证中文路径、取消复制、只读安装目录和失效最近项目。

### Roadmap Evidence Index（A1～A22）

- **本日覆盖**：`A20 演示项目`。
- **反向证据**：安装态复制、打开、预览、导出、取消目录选择与只读模板证据。

### 追踪来源

- [原正式 Day 36 工单](../../../agent%20task/DAY-36-AGENT-TASK.md)
...
- **预计变更文件**：
  ...
  - `src/renderer/features/onboarding/WelcomePage.tsx`
  - `src/renderer/features/onboarding/FirstRunGuide.tsx`
  - `src/main/services/DemoProjectService.ts`
  - Preload/IPC 必要适配
...
git rev-parse HEAD
git status --short
git grep -n "DemoProjectService\|FirstRunGuide\|WelcomePage\|LICENSES" -- src demo-project resources tests docs
pnpm typecheck
pnpm lint
pnpm test:unit
