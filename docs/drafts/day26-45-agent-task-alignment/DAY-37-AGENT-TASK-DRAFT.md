...
> **工单编号**：B-37/45<br>
> **角色**：Engineer<br>
> **来源**：`DAILY_PLAN.md` Day 37<br>
> **分支建议**：`test/day-37-regression-suite`<br>
> **任务类型**：测试建设 + 桌面 E2E + CI 分层<br>
...
**整合验收 / 自动护栏。** 本日不新增产品 UI；扩展 Day 26 已建立的 runtime component 测试层，并建立真实 Electron E2E。

### 必须固化的工作区合同

- Unit：workspace reducer/store、选择优先级、time geometry/snap/format、Inspector context、响应式模式、draft key/reset、export state/progress。
- Component：Activity 切换只有一个主 Manager、Inspector 随选择切换、Timeline UI 操作不 dirty、草稿不串项目、Dialog focus/Escape/aria、空状态与错误消息。
- Electron E2E：800×560、1200×760、1366×768；根无滚动；Canvas/History/Timeline 单实例；镜头→图层→动作→播放；素材导入→拖放；对白→字幕→音频；保存重开；短导出。
- E2E 必须通过用户可见 UI，不允许直接写 Store/JSON 代替流程。

### 证据与边界

- 失败自动保存 screenshot、trace、renderer console 与 main log，并关联到同一 commit。
- 视觉回归以 `DESIGN.md` 的数值和状态合同为准，参考图只作方向，不做像素神谕。
- 自动化不得为兼容旧 Gate 保留隐藏的第二套 DOM；selector owner 与 cardinality guard 必须指向正式工作区。

### 性能与视觉回归预算

- 先记录硬件、采样方法和真实基线；Activity/Inspector 切换到可交互的 p95 目标均为 `< 100 ms`，Canvas 与 Timeline 拖动目标均为 `>= 55 FPS`。
- Export IPC 用户态更新最多约 10 次/秒；100 个素材卡滚动无明显输入阻塞；连续 20 次 Activity/Dock 切换后订阅/监听数量稳定；连续 5 次 Preview 后 AudioContext/AudioNode 回落。
- 固定截图矩阵必须覆盖 800×560、1024×640、1200×760、1366×768、1920×1080，以及 150% DPI。
- 每档至少覆盖 StartScreen、空项目、选中图层、打开 Timeline、选中 Dialogue、Export Dialog 的 validating/rendering/failed/completed 与 Recovery Banner。
- 视觉差异只能 mask 动态时间码和文件路径；DOM cardinality、根滚动、Dock/Stage/Dialog 尺寸和可达性必须有独立结构断言，不能只靠像素截图或主观“顺滑”。

### Roadmap Evidence Index（A1～A22）

- **本日覆盖**：`A1 项目生命周期`、`A3 自动保存与恢复`、`A4 素材导入`、`A8 撤销重做`、`A21 自动化测试`。
- **反向证据**：runtime component + Playwright Electron E2E，覆盖新建/保存/恢复、UI 主路径、唯一 Owner 与失败证据包。

### 追踪来源

- [原正式 Day 37 工单](../../../agent%20task/DAY-37-AGENT-TASK.md)
