...
> **工单编号**：B-42/45<br>
> **角色**：Engineer<br>
> **来源**：`DAILY_PLAN.md` Day 42<br>
> **分支建议**：`fix/day-42-top-production-blockers`<br>
> **任务类型**：P0 Bug 修复 + 回归测试 + Gate C 复验<br>
...
**整合验收 / 证据驱动 P0。** 只允许 Sample A 证据中的前三个真实生产阻塞进入；不借机进行视觉重构。
...
“颜色不够漂亮”或“间距想再调”不是 P0；不新建设计 Token、不恢复 LegacyWorkspace、不增加第二套 UI Owner。

### 每项修复的补充验收

- 明确归属现有 TopBar / Dock / Stage / Inspector / Timeline / Dialog / Main service 中的唯一 Owner。
- 补复现与回归，覆盖项目/UI/后台状态边界、dirty/History、单实例与旧路径退出。
- 若涉及布局或可达性，在 800×560、1366×768 和相关 DPI 做真实 Electron 回归。
- 证明修复直接减少 Sample A 的阻塞，并复验 Gate C；不相关发现进入 Backlog。

### Roadmap Evidence Index（A1～A22）

- **本日覆盖**：`A17～A19` + 每个真实 P0 实际影响的 `Axx`。
- **反向证据**：每项修复的复现/回归必须标注受影响 A 编号；随后复验 Gate C 的视频导出、一致性与取消。

### 追踪来源

- [原正式 Day 42 工单](../../../agent%20task/DAY-42-AGENT-TASK.md)
