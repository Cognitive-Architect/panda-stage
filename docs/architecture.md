...
Main Process
  ...
  └─ FFmpegAdapter：版本/编码器检查、静音 H.264 编码与 ffprobe
...
`FFmpegAdapter` 只在 Main Process 使用 `spawn(executable, args)` 启动外部进程，固定 `shell:false`。它在编码前验证连续帧、输出目录、FFmpeg 版本和 `libx264`，编码后通过 ffprobe 验证真实媒体流。Renderer 和 Preload 不暴露任何子进程能力。

Day 07 输出固定为静音 H.264/yuv420p MP4；AAC、音频 mux、sidecar 打包和正式导出 UI 不在本层。外部工具路径来自显式配置或开发期环境变量，仓库不包含二进制。
