...
Day 07 输出固定为静音 H.264/yuv420p MP4；Day 08 在同一 Adapter 内用整数 `startMs` 和 `adelay` 合成单条 WAV，视频流保持 H.264，音频编码为 AAC。当前不实现多轨 `amix`、sidecar 打包或正式导出 UI。外部工具路径来自显式配置或开发期环境变量，仓库不包含二进制。
