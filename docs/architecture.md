...
完整探针依次执行 `preparing → rendering/writing → encoding → muxing → cleaning → finished`。项目目录先在 Main Process 以 Node 路径 API 验证可读，音频和输出路径始终作为独立参数传递。真实 Windows 验证覆盖中文、空格和 emoji 路径，详见 Day 09 回执。
