...
- `adelay` 列表按 ffprobe 返回的声道数逐项重复请求中的 `startMs`，视频使用 `-c:v copy` 保留 H.264，音频编码为 AAC 192 kbps；
...
- `pnpm typecheck`：通过；
- `pnpm lint`：通过；
- `pnpm test:unit`：12 个测试文件、75 项测试通过；
...
## 2026-07-22 pre-PR 修复

- mux 构造参数前复用 `probeAudioFile()` 获取声道数，并拒绝非正整数声道元数据；损坏或不可解码音频仍返回 `AUDIO_INPUT_INVALID`，不会启动 mux 进程；
- 旧 FFmpeg 兼容参数按声道展开：mono/400ms 为 `adelay=400`，stereo/400ms 为 `adelay=400|400`，stereo/0ms 为 `adelay=0|0`；
- `MuxProbeExpectationSchema` 与 `assertMuxProbeMatches()` 增加帧数硬门禁，Day 08 真实验证固定要求 72 帧；
- 视频复制、AAC 192k、同目录临时 MP4 提交、完整音频保留和单 WAV 范围保持不变。

修复后 `pnpm typecheck`、`pnpm lint`、`pnpm test:unit`、`pnpm build`、Day 03～05、Day 06 失败路径与完整 `pnpm verify:day08` 均通过。FFmpeg 音频 mux 定向测试与 Day 07 Adapter 回归共 31 项通过；真实 mono 探针仍生成 `adelay=400`，输出仍为 72 帧，三次声音起点仍均为 0.400646 秒。
