...
| 次数 | 阶段 | 耗时 | 被终止编码 PID | 进程/窗口/目录/写入残留 | 无关 sentinel |
|---:|---|---:|---:|---|---|
| 1 | rendering | 323ms | — | 0 / 0 / 0 / 0 | 存活 |
| 2 | rendering | 285ms | — | 0 / 0 / 0 / 0 | 存活 |
| 3 | writing | 9ms | — | 0 / 0 / 0 / 0 | 存活 |
| 4 | encoding | 243ms | 10356 | 0 / 0 / 0 / 0 | 存活 |
| 5 | encoding | 237ms | 9388 | 0 / 0 / 0 / 0 | 存活 |

每次错误均包含对应 Job ID，以及“已完成资源清理；可以立即重新导出”的用户操作建议。编码取消使用真实 FFmpeg 进程；独立 sentinel 在全部取消期间保持运行，证明未影响无关进程。

## 取消后恢复导出

- 真实共享 Canvas 捕获：72 帧，143052ms；完整流水线 146245ms。
...
- 完成后：活动 Job 0、编码进程 0、隐藏窗口 0、帧目录 0、在途写入 0。
...
| FUNC-004 | 通过 | 第 4、5 次真实 FFmpeg PID 取消 |
...
- `pnpm typecheck`：通过。
- `pnpm lint`：通过。
- `pnpm test:unit`：13 个测试文件、85 项测试通过。
- `pnpm build`：通过；仅保留既有 Vite 大 chunk 警告。
- `pnpm verify:day09`：通过；包含构建、5 次取消、恢复导出、ffprobe、Chromium 和资源计数。
