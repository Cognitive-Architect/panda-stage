...
| `pnpm typecheck` | PASS |
| `pnpm lint` | PASS |
| `pnpm test:unit` | PASS：37 files / 215 tests |
| `pnpm test:integration` | PASS：5 files / 47 tests |
| `pnpm build` | PASS |
...
- DECISION-003：临时复制、flush、排他提交、立即登记 rollback candidate、一次模型保存；构造或保存失败回滚本事务新文件。
- DECISION-004：Main 的 Autosave 会话是 revision 权威；项目 ID、完整快照或 revision 任一不匹配均以 `ASSET_IMPORT_STALE_REVISION` 拒绝。
...
| 完成标准 | 状态 | 权威证据 |
|---|---|---|
| Main 在复制前拒绝 stale revision | PASS | stale 集成测试断言 `beforeCopy = false` |
| 相同 revision 并发导入不覆盖 | PASS | 两个请求同时发出；第二个 stale，刷新重试后 revision 2 且两个 Asset 均存在 |
| 同内容并发不重复 | PASS | 第二个 stale，刷新重试返回 `duplicate`；1 文件/1 Asset |
| 陈旧导入不覆盖其他编辑 | PASS | 导入前及导入中 revision 前进均返回 stale；项目名称、dirty store 与正式 JSON 保持 |
| 文件落盘后立即进入回滚责任 | PASS | `afterFileRegistered` 故障注入后正式文件数与 `.tmp` 均为 0 |
| AssetSchema / ProjectSchema 异常清理 | PASS | 无效 Asset ID 与 `beforeProjectBuild` 三文件故障测试 |
| 超长 Unicode 文件名 | PASS | 205 字符源 stem；Asset 名 200，目标 stem 120，保存重开一致 |
| 所有普通失败 `.tmp = 0` | PASS | 每个故障测试枚举 `assets/`，无 `.asset-import.*.tmp` |
| 回滚删除失败明确报告 | PASS | `ASSET_IMPORT_ROLLBACK_FAILED` 携带唯一正式残留路径 |
| Renderer stale 不改 store | PASS | dirty revision 1 单元测试前后深比较相同 |
| Day 16 原能力 | PASS | 四媒体、三重校验、SHA-256、去重、冲突、Unicode、相对路径、源删除测试 |

请求采用明确的混合结果策略：可预期的单文件校验/复制失败不会撤销同一批次中其他成功项；任何复制后的内部模型构造、revision 冲突或项目保存异常都会清理本请求尚未提交的全部正式文件。每个成功结果必须同时存在 Asset 和正式文件。
...
| 完成标准 | 状态 | 权威证据 |
|---|---|---|
| CleanupError 在真实复制边界识别 | PASS | `copyIntoAssetsAtomically()` catch 提升为 rollback failed；hash catch 无误导分支 |
| target + temporary 同时残留 | PASS | link 后首次 unlink 失败，内部两项清理均故障；返回两条实际路径 |
| 内部清理全部成功 | PASS | 首次 unlink 失败但 target/temp 清理成功；普通 `ASSET_IMPORT_COPY_FAILED`，目录为空 |
| 仅 temporary 残留 | PASS | target 清理成功、temporary 清理失败；只返回 `.asset-import.*.tmp` |
| 项目与 Asset 不变 | PASS | 三场景均断言 `project.json` SHA-256 与 Asset 数量不变 |
| IPC 路径不丢失 | PASS | IPC 单元测试保持 `ASSET_IMPORT_ROLLBACK_FAILED` 与原 residualPaths |
| Renderer 可操作提示 | PASS | 真实 Electron 证据与单元测试包含“导入清理未完成”及每条路径，不含普通复制失败文案 |
| Store / dirty / revision 不变 | PASS | rollback failure 响应前后 store 深比较一致 |
| Issue #26 与 Day 16 回归 | PASS | 215 个单元测试、47 个集成测试及完整门禁 |

UI 证据：`docs/evidence/day-16/cleanup-failure.png`；机器证据：`docs/evidence/day-16/results.json`。
