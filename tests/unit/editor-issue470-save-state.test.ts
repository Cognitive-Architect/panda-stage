...
describe('Issue #470 Quick Action Drawer save-state presentation', () => {
  it('shows the rest-state only while the drawer is collapsed and unsaved', () => {
    expect(shouldRenderRestSaveState(false, 'dirty')).toBe(true);
  ...
  it('keeps the in-drawer state and removes the duplicate external surface', () => {
    ...
    expect(dirty).toContain('data-testid="project-save-state"');
