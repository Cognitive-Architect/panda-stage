...
async function snapshot(window) {
  return window.webContents.executeJavaScript(`(() => {
    ...
    return {
      ...
      backgroundRender: {
        x: Number(stage?.dataset.backgroundRenderX),
        y: Number(stage?.dataset.backgroundRenderY),
        width: Number(stage?.dataset.backgroundRenderWidth),
        height: Number(stage?.dataset.backgroundRenderHeight),
        offsetX: Number(stage?.dataset.backgroundRenderOffsetX),
        offsetY: Number(stage?.dataset.backgroundRenderOffsetY),
        scaleX: Number(stage?.dataset.backgroundRenderEffectiveScaleX),
        scaleY: Number(stage?.dataset.backgroundRenderEffectiveScaleY),
      },
      renderedAssetIntrinsicSizes: JSON.parse(stage?.dataset.renderedAssetIntrinsicSizes || '[]'),
