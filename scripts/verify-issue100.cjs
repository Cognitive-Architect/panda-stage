...
async function readLogicalStageRect(window) {
  return window.webContents.executeJavaScript(`(() => {
...
async function captureLogicalStage(window, rect) {
  progress(`capturing logical stage: ${JSON.stringify(rect)}`);
  const png = (await window.webContents.capturePage(rect)).toPNG();
  progress('logical stage captured');
  return {
    rect,
    png,
  };
...
async function clickViewportChrome(window) {
  const client = await window.webContents.executeJavaScript(`(() => {
    ...
    if (!viewport || !stage) {
      throw new Error('Canvas viewport chrome did not render.');
    }
    const rect = viewport.getBoundingClientRect();
    const visibleLeft = Math.max(0, Math.ceil(rect.left));
    const visibleTop = Math.max(0, Math.ceil(rect.top));
    const visibleRight = Math.min(window.innerWidth, Math.floor(rect.right));
    const visibleBottom = Math.min(window.innerHeight, Math.floor(rect.bottom));
    const candidates = [];
    for (let y = visibleTop + 4; y < visibleBottom - 4; y += 8) {
      for (let x = visibleLeft + 4; x < visibleRight - 4; x += 8) {
        candidates.push({ x, y });
      }
    }
    const point = candidates.find(({ x, y }) => {
      const target = document.elementFromPoint(x, y);
      return target && viewport.contains(target) && !stage.contains(target);
    });
    if (!point) throw new Error('No visible viewport chrome point was found.');
    ...
    return {
      ...
      viewportContainsTarget: Boolean(target && viewport.contains(target)),
      stageContainsTarget: Boolean(target && stage.contains(target)),
    };
  })()`);
...
async function run(window) {
  const checks = [];
  Object.defineProperty(checks, 'push', {
    value(...items) {
      progress(`checks passed: ${items.join(', ')}`);
  ...
  progress('creating project A fixture');
  const fixtureA = await createFixture(parent, 'a');
  progress('creating project B fixture');
  ...
  progress('project A assets ready');
  const logicalStageCaptureRect = await readLogicalStageRect(window);
  progress(
    `logical stage capture rect ready: ${JSON.stringify(logicalStageCaptureRect)}`,
  );
  ...
  const viewportChromeClick = await clickViewportChrome(window);
  progress(`viewport empty point clicked: ${JSON.stringify(viewportChromeClick)}`);
  const unselectedBackground = await snapshot(window);
  progress(
    `viewport deselection snapshot read: ${JSON.stringify({
      selectedLayerId: unselectedBackground.selectedLayerId,
      inspectorState: unselectedBackground.inspectorState,
    })}`,
  );
  ...
  const beforeBackgroundSelection = await captureLogicalStage(
    window,
    logicalStageCaptureRect,
  );
  ...
  const afterBackgroundSelection = await captureLogicalStage(
    window,
    logicalStageCaptureRect,
  );
  const unselectedBackgroundLayer = unselectedBackground.layers.find(
    (layer) => layer.id === fixtureA.layerA.id,
  );
  const selectedBackgroundLayer = managedLocked.layers.find(
    (layer) => layer.id === fixtureA.layerA.id,
  );
  const backgroundGeometryFields = [
    'x',
    'y',
    'scaleX',
    'scaleY',
  ...
  assert(
    unselectedBackgroundLayer &&
      selectedBackgroundLayer &&
      backgroundGeometryFields.every(
        (field) =>
          unselectedBackgroundLayer[field] === selectedBackgroundLayer[field],
      ),
    `Selecting the formal background changed its model geometry: ${JSON.stringify({
      before: unselectedBackgroundLayer,
      after: selectedBackgroundLayer,
    })}`,
  );
  ...
  assert(
    backgroundSelectionPixels.sameSize &&
      backgroundSelectionPixels.differenceRatio <= 0.003,
