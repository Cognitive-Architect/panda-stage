...
async function run(window) {
  ...
  const formalBackgroundLayer = formalBackground.layers.find(
    (layer) => layer.id === createdLayer.id,
  );
  assert(
    reopenedA.layers.some(
      (layer) =>
        layer.id === createdLayer.id &&
        formalBackgroundLayer &&
        layer.x === formalBackgroundLayer.x &&
        layer.y === formalBackgroundLayer.y &&
        layer.scaleX === formalBackgroundLayer.scaleX &&
        layer.scaleY === formalBackgroundLayer.scaleY &&
        layer.rotationDeg === formalBackgroundLayer.rotationDeg &&
        layer.opacity === formalBackgroundLayer.opacity &&
        layer.locked === formalBackgroundLayer.locked &&
        layer.flipX === formalBackgroundLayer.flipX,
    ),
    `Save/reopen lost the formal background Cover geometry contract: ${JSON.stringify({
      formalBackground: formalBackgroundLayer,
      reopened: reopenedA.layers.find(
        (layer) => layer.id === createdLayer.id,
      ),
    })}`,
  );
